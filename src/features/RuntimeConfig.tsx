...
export default function RuntimeConfig() {
  ...
  const saveProfile = async () => { if (!profile) return; await governanceApi.saveProfile({ ...profile, config: typeof profile.config === 'string' ? JSON.parse(profile.config) : profile.config }); setProfile(null); profileState.reload() }
